This repository contains `zmk-feature-os-detection`, a ZMK module that detects
the host OS (Windows / macOS·iOS / Linux / unknown) over USB and BLE, exposes
a Custom Studio RPC subsystem (`cormoran__os_detection`), and provides a Web
UI to view detection state and override the BLE per-profile OS classification.
It uses the **unofficial** custom ZMK Studio RPC protocol from
`cormoran/zmk-module-template-with-custom-studio-rpc`.

License is MIT. This module must never reference, copy, or adapt QMK
(GPL-2.0) source code.
